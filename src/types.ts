...
export interface AccountProgress {initialBand:number;logs:Log[];active:Active|null;deferred:Record<string,number>}
export interface State {version:1;duration:number;initialBand:number;profile:Profile;logs:Log[];active:Active|null;deferred:Record<string,number>;catalog?:Catalog;bands?:Record<string,number[]>;accounts?:Record<string,AccountProgress>}
...
declare global { interface Window {fieldwork?:{load:()=>Promise<string|null>;save:(json:string)=>Promise<void>;sync:(handle:string)=>Promise<Profile>;catalog:()=>Promise<Catalog>;open:(url:string)=>Promise<void>;check:(code:string)=>Promise<CompileResult>;copy:(code:string)=>Promise<void>;readClipboard:()=>Promise<string>}} }
